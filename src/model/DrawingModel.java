...
public class DrawingModel {
    private final String namePrizesToAward; // название файла - призы вручить
    private final String namePrizesAwarded; // название файла - призы врученные
    DateTimeFormatter formatter; //формат даты времени
    private Drawing drw;   //Данные о розыгрыше призов

    public DrawingModel() {
        namePrizesToAward = "src/prizestoaward.csv";
        namePrizesAwarded = "src/prizesawarded.csv";
        drw = new Drawing();
    ...
    public boolean loadPrizesToAward() {
        ...
        try (FileReader fr = new FileReader(namePrizesToAward)) {
    ...
    public boolean loadPrizesAwarded() {
        ...
        // открываем и читаем данные из файла
        try (FileReader fr = new FileReader(namePrizesAwarded)) {
            Scanner scanner = new Scanner(fr);
            int i = 0; // номер строки
            while (scanner.hasNextLine()) {
                /*
    ...
    public boolean savePrizesToAward() {
        ...
        try {
            FileWriter fr1 = new FileWriter(namePrizesToAward);
    ...
    public boolean savePrizesAwarded() {
        // сохранение списка в файл БД
        try {
            FileWriter fr1 = new FileWriter(namePrizesAwarded);
            //записываем шапку таблицы
